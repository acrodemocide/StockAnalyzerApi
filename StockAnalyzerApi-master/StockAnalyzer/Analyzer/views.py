...
Folios = [
    ...
    DataObject(id=3, name='conservative', b_and_h_value='11241', tactical_rebal_value='4444', 
              b_and_h_allocation=[200, 100, 700], tactical_rebal_allocation=[334, 333, 333], 
...
class PortfolioViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        ...
        for i in range(0,len(full_list)):
            print('full_list[i]: ', full_list[i])
            print('type(threaded_list[i][0]): ', type(threaded_list[i][0]))
            print('len(threaded_list[i][0]): ', len(threaded_list[i][0]))
            print('threaded_list[i]: ', threaded_list[i][0])
            stock_objects_list.append(Stock(full_list[i], threaded_list[i][0]))
        """List of stock objects stored in stock_objects_list. None 
