...
# Function replaced in multi-threading section:
"""def organize_data_func(arr):
    ...
    for i in stock_list:
        ...
        print('1')
        print('count: ', count)
        data = stock.history(period="max")#, interval="1wk")#######
...
def extract(stock_symbol):
    ...
    data = stock.history(period="max", interval="1wk") # updated interval to 1wk to minimize api requests
    close = data["Close"]
